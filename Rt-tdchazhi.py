...
FigureName = 'Rt_td_chazhi_60min.jpg'
...
# set_V = [1.1]
set_V = [0.249,0.3,0.35,0.4,0.5,0.7,1.1,2.2,3.5]
...
for i in range(len(td_list)-1):
    for j in range(i+1,len(td_list)):
        if td_list[j]<=td_list[i]:
            td_list[i],td_list[j] = td_list[j],td_list[i]
            Rt_list[i],Rt_list[j] = Rt_list[j],Rt_list[i]

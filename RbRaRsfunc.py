...
def xy_latlong(x,y,sstep):
    ...
    LAT = w00*LAT00+w01*LAT01+w10*LAT10+w11*LAT11
    LON = w00*LONG00+w01*LONG01+w10*LONG10+w11*LONG11
    return  LON, LAT
...
for sh in set_height: 
    for sv in set_V:
        ...
        for st in range(len(set_t)):
            ...
            y,y_title = 70,70
            #z = 9445.736
            z = sh + df['HGT'][1][int(y)][int(x)]  #排放源的高度，单位为m
            ...
            h0 = 0
            H = []
            for i in range(df['H'].shape[1]):
                z0 = df['H'][t_h][i][int(y)][int(x)]
                h0 = h0 + np.array(z0)  #累加高度h0
                H = np.append(H,h0)
            H = H.tolist()
            print('在开始时形成一个高度 H list')
            print()
            while z > df['HGT'][1][int(y)][int(x)]:
                ...
                for p in xyz_0:
                    for q in xyz_1:
                        for k in xyz_2:
                            ...
                            U.append(math.sqrt((df['U'][t_h][k][q][p])**2*b)/abs(df['U'][t_h][k][q][p])*df['U'][t_h][k][q][p])
                            V.append(math.sqrt((df['V'][t_h][k][q][p])**2*b)/abs(df['V'][t_h][k][q][p])*df['V'][t_h][k][q][p])
                            W.append(df['W'][t_h][k][q][p]*b)
                u = sum(U)
                v = sum(V)
                w = sum(W)
                ws=np.sqrt(u*u+v*v)
